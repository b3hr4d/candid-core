...
A merged `Cargo.lock` change makes comparisons drift-informational until the
documented recapture (dispatch `Verify`, review the artifact's
`baseline.json`, commit it to `benches/baselines/main.json`).

...
- Adversarial tests showing every derived provenance collection must match
  compiler rederivation from the embedded source bundle (`compiler` feature).
- Public-API tests pinning which of these types each feature exports
  (`tests/model_public_api.rs`).

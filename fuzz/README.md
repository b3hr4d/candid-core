...
Install `cargo-fuzz`, then run each trust-boundary target with a bounded time
budget:
...
The scheduled CI job runs the same targets. Minimized crash inputs belong in
the relevant target corpus and require a deterministic regression test.
